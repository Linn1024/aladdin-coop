...
m68ki_cpu_core m68k;
#include "aladdin_coop.h"
static unsigned int probe_trace[2000000];
static unsigned int probe_count, probe_enabled;
__declspec(dllexport) void al_probe_start(void) { probe_count=0; probe_enabled=1; }
__declspec(dllexport) unsigned int al_probe_stop(unsigned int **p) { probe_enabled=0; *p=probe_trace; return probe_count; }
__declspec(dllexport) unsigned int *al_probe_regs(void) { return m68k.dar; }
...
{
  extern void (*zt_sync)(unsigned int);
  extern void zt_poll(void);
  ...
  {
    if (zt_sync) { zt_sync(m68k.cycles); zt_poll(); }
    ...
    al_coop_hook();
    if (probe_enabled && probe_count < 2000000) probe_trace[probe_count++]=REG_PC;
